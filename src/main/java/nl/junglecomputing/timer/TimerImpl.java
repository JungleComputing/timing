...
public class TimerImpl implements java.io.Serializable, nl.junglecomputing.timer.Timer {
    ...
    private static class TimerEvent implements java.io.Serializable, Comparable<TimerEvent> {
        ...
        public TimerEvent(String node, String device, String thread, String action, long queued, long submitted, long start, long end) {
        ...
        public String toDataTransferString() {
            return String.format("%-8s  |  %-14s  |  start: %-6s  |  " + "end: %-6s  |  duration: %-6s  | nrBytes: %4.3f MB  |  " + "rate: %13s\n", node,
                    action, format(start / 1000.0), format(end / 1000.0), format((end - start) / 1000.0), nrBytes / 1024.0 / 1024.0, getRateString());
        }

        @Override
        public String toString() {
            return String.format("%-8s  |  %-10s  |  %-22s  | " + "%-14s  |  queued: %-6s  |  submitted: %-6s  |  " + "start: %-6s  |  end: %-6s\n", node,
                    device, thread, // queueIndex,
                    action, format(queued / 1000.0), format(submitted / 1000.0), format(start / 1000.0), format(end / 1000.0));
        }
    ...
    public void append(StringBuffer sb, String node, String device, String thread, long start, long end, String action, boolean perThread) {
        sb.append(String.format("%s %s %s\t%f\t%f\t%s\n", node, device, perThread ? thread : "nothread", start / 1e6, end / 1e6, action));
    }
    ...
    public String gnuPlotData(boolean perThread) {
        ...
        for (TimerEvent event : events) {
            if (event.getEnd() > 0) {
                append(sb, event.getNode(), event.getDevice(), event.getThread(), event.getStart(), event.getEnd(), event.getAction(), perThread);
            }
        }
        return sb.toString();
    }
